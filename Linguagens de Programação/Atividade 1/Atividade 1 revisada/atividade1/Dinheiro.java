...
public class Dinheiro {
	...
	//não utilizou os atributos de taxa, então por que atualizar as mesmas atraves de uma função se não são utilizadas?
    ...
    public double valorEmEuro(double valor1){
        double valor;
        if(this.getMoeda() == 'r'){
            valor = this.realParaEuro(valor1); // não existe realParaEuro e não foi implementado
        }
        else if(this.getMoeda() == 'd'){
